...
namespace NLRBScraper
{
    internal class Program
    {
        ...
        {
            var retInit = GetInitial().Result;
            Pass2(retInit).Wait();
            Console.WriteLine("Hello, World!");
        }

        static async Task<(string formBuildId, string formId)> GetInitial()
        {
        ...
        static async Task Pass1((string formBuildId, string formId) input)
        {
            using var httpClient = new HttpClient();
        ...
        static async Task Pass2((string formBuildId, string formId) input)
        {
            using var httpClient = new HttpClient();
